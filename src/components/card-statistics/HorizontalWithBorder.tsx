...
const HorizontalWithBorder = (props: CardStatsHorizontalWithBorderProps & { index: number }) => {
  // Props
  const { title, stats, trendNumber, avatarIcon, color, index } = props
      ...
      <CardContent className='flex flex-col gap-1'>
        ...
        <div className='flex flex-col gap-1'>
          ...
          <div className='flex items-center gap-2'>
            ...
            >{`${trendNumber > 0 ? '+' : ''}${trendNumber} ${index === 3 ? '' : '%'}`}</Typography>
            <Typography variant='body2' color='text.disabled'>
              {index == 2 ? 'Next 7 days (3) today' : index === 3 ? 'Out of 5.0' : 'than last week'}
            </Typography>
          </div>
        </div>
      </CardContent>

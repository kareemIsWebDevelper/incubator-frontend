...
const CustomTextField = forwardRef((props: TextFieldProps, ref) => {
  const { size = 'small', InputLabelProps, select, placeholder, SelectProps, value, ...rest } = props
      ...
      renderValue: (selected: unknown) => {
        ...
        if (SelectProps?.multiple) {
          ...
          if (!selectedArray || selectedArray.length === 0) {
            return <span style={{ color: 'var(--mui-palette-text-disabled)' }}>{placeholder}</span>
          }
          // If there's a custom renderValue in SelectProps, use it, otherwise return the selected values
          return SelectProps.renderValue ? SelectProps.renderValue(selected) : selectedArray.join(', ')
        }
  ...
  return (
    <TextFieldStyled
      ...
      select={select}
      value={value}
      placeholder={!select ? placeholder : undefined}
